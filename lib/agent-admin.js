...
═══════════ ХИЙГЭЭГҮЙ ЗҮЙЛЭЭ "ХИЙСЭН" ГЭЖ БҮҮ ХЭЛ ═══════════
Энэ бол хамгийн ноцтой алдаа. Эзэн хийгдсэн гэж итгээд өнгөрөхөд
хэрэглэгч хариугаа хэзээ ч авахгүй үлдэнэ.

- Хэрэгслээ дуудаж, буцаасан хариултыг нь ХАРАЛГҮЙГЭЭР "орууллаа",
  "хаалаа", "илгээлээ" гэж ХЭЗЭЭ Ч бүү хэл. Зөвхөн хэрэгслийн бодит
  хариултад тулгуурла.
- "Аль хэдийн хийчихсэн байна" гэж хэлэхээсээ ӨМНӨ list_unknowns-оор
  ЗААВАЛ шалга. Асуултын ТЕКСТ нь таарч байгаа эсэхийг хар.
  Дугаар нь ойрхон, сэдэв нь төстэй гэдгээр өөр асуултыг хутгаж
  болохгүй — "англи хэл заадаг уу" ба "өөр салбартай юу" хоёр нь
  ӨӨР асуулт.
- Илгээлт амжилтгүй болсон гэж хэлэхийн тулд resolve_unknown эсвэл
  deliver_answer-ийн буцаасан АЛДААГ бодитоор харсан байх ёстой.
  Facebook-ийн хугацааны хязгаарыг таамаглаж шалтаг болгож болохгүй.
- Эргэлзвэл шалга. Шалгах нь таамаглахаас үргэлж дээр.
...
const TOOLS = [
  ...
  },
  {
    name: "delete_unknown",
    description:
      "Мэдэхгүй асуултын бичлэгийг жагсаалтаас БҮРМӨСӨН устгана. " +
      "Мэдээллийн санг ХӨНДӨХГҮЙ — үнэ, багш, хаяг зэрэг орсон мэдээлэл " +
      "хэвээрээ үлдэнэ, зөвхөн асуултын бичлэг арилна. Эзэн 'энэ " +
      "асуултыг устга', 'жагсаалтаа цэвэрлэ' гэх мэт зүйл хэлвэл ашигла.",
    input_schema: {
      type: "object",
      properties: {
        ids: {
          type: "array",
          items: { type: "number" },
          description: "Устгах асуултын дугаарууд",
        },
      },
      required: ["ids"],
    },
...
const HANDLERS = {
  ...
  async delete_unknown(input) {
    const ids = input.ids ?? [];
    if (!ids.length) return "Ямар дугаартай асуултыг устгахыг хэлээгүй байна.";

    const removed = [];
    const missing = [];
    for (const id of ids) {
      const record = await deleteUnknown(id);
      (record ? removed : missing).push(id);
    }

    const parts = [];
    if (removed.length) {
      parts.push(
        `${removed.map((id) => `#${id}`).join(", ")} устлаа. ` +
          "Мэдээллийн санд орсон мэдээлэл хэвээрээ.",
      );
    }
    if (missing.length) {
      parts.push(`${missing.map((id) => `#${id}`).join(", ")} олдсонгүй.`);
    }
    return parts.join(" ");
  },
