...
// Хаагдсан боловч хариу нь хүнд хүрээгүй асуултууд. Илгээлт ямар нэг
// шалтгаанаар (алдаа, тасалдал) хийгдээгүй үлдвэл эндээс олж илгээнэ.
const PENDING_KEY = "unknowns:undelivered";
...
export async function resolveUnknown(id, answer) {
  ...
  if (!record.deliveredAt) await redis().sadd(PENDING_KEY, id);
  return next;
}
...
export async function markUnknownDelivered(id, ok) {
  ...
  await redis().set(`unknown:${id}`, next);
  // Амжилтгүй болсныг ч жагсаалтаас хасна — эцэс төгсгөлгүй дахин
  // оролдохгүйн тулд. Эзэн deliver_answer-ээр гараар дахин илгээж болно.
  await redis().srem(PENDING_KEY, id);
  return next;
}

/** Илгээгдээгүй үлдсэн асуултын дугаарууд. */
...
/** Хуучин бичлэгийг жагсаалтад нэмэх (нөхөж бүртгэх). */
export async function markPendingDelivery(id) {
  await redis().sadd(PENDING_KEY, id);
}

/** Илгээх боломжгүй болсон бичлэгийг жагсаалтаас хасна. */
export async function clearPendingDelivery(id) {
  await redis().srem(PENDING_KEY, id);
}

/**
 * Мэдэхгүй асуултын бичлэгийг бүрмөсөн устгана.
 * Мэдээллийн санг (kb) ХӨНДӨХГҮЙ — тэр өөр түлхүүрт хэвтдэг.
 */
export async function deleteUnknown(id) {
  const record = await getUnknown(id);
  await redis().del(`unknown:${id}`);
  await redis().zrem("unknowns", id);
  await redis().srem(PENDING_KEY, id);
  return record;
}
